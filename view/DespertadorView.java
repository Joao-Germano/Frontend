...
public class DespertadorView {
    ...
    public static void main(String[] args) {
        ...
        while (sair == false) {
            DespertadorController.getHMS();
            int minutoRestante = minutoDespertar - minutoAtual - 1;
            int horaRestante = horaDespertar - horaAtual;
            int segundoRestante = segundoDespertar - segundoAtual ;
            horaRestante = (minutoRestante >= 0) ? horaRestante : horaRestante -1;
            minutoRestante = (minutoRestante >= 0) ? minutoRestante : 60 + minutoRestante;
            segundoRestante = (segundoRestante >= 0) ? segundoRestante : 60 + segundoRestante;
            
            System.out.println(
                String.format(
                    "Agora são: %s%02d:%02d:%02d%s. O próximo alarme irá despertar às %s%02d:%02d%s. Adiado %s%d%s vez(es). O tempo restante para despertar é: %02d horas e %02d minutos %02d segundos ", 
                    ANSI_YELLOW,
                    horaAtual, 
                    minutoAtual, 
                    segundoAtual,
